...
- Рекомендация: Добавить стандартный блок Verbosity Protocol из qa_agent.md § Skill Completion Protocol
...
- **Исключения:**
  - `audit/audit-history.md` — централизованный append-only лог. Не флагать
...
### Check 10: History Append

После формирования отчёта — append строку в `audit/audit-history.md`:

```
| {YYYY-MM-DD} | /skill-audit | {scope} | — | {CRITICAL} | {WARNING} | {INFO} | ~{estimated token savings} | {краткое примечание} |
```

- Score: `—` (skill-audit не генерирует единый score)
- Тесты: estimated token savings из Summary
...
| # | Скилл | Check | Severity | Описание | Рекомендация |
|---|-------|-------|----------|----------|--------------|
| 1 | /api-tests | Self-Review | CRITICAL | 77-строчный шаблон + файл | → Post-Check inline |
| 2 | /api-tests | Tech Stack | WARNING | Дублирует CLAUDE.md | → Ссылка |
...
| WARNING | N |
| INFO | N |
| Estimated token savings | ~N tokens/invocation |
```

Для каждого скилла также вывести:
```
{skill-name}: {lines} строк → {target} (если нужна оптимизация)
```
